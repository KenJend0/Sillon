// Miroir de apps/web/lib/buildAddQueue.ts — module pur, logique identique.
// Mode dégradé (Phase 6.4, avant Phase 7 "Explore" côté mobile) : forYouAlbums,
// forYouTracks et discoveryAlbums sont toujours des tableaux vides pour l'instant
// (voir docs/MOBILE_ROADMAP.md, notes de scope 6.4) — les types sont conservés à
// l'identique du web pour que buildAddQueue n'ait rien à changer quand Phase 7
// branchera les vraies données.
import { type ListAlbumItem, type ListTrackItem, type UnratedItem } from './lists';
import { CLASSIC_ALBUMS } from './classicAlbums';

export type ForYouAlbum = { album_id: string; title: string; artist: string; cover_url: string };
export type ForYouTrack = { track_id: string; track_title: string; artist: string; album_id: string; artist_id: string; cover_url: string | null };
export type DiscoveryAlbum = { album_id: string; title: string; artist: string; cover_url: string; via_username?: string | null };
...
/**
 * File pour l'onglet Ajouter : priorise ce qui a déjà été ajouté à une liste mais
 * jamais noté, puis la liste sélectionnée, puis les suggestions personnalisées, et
 * complète avec des classiques jusqu'à MIN_QUEUE_SIZE si le signal réel ne
 * suffit pas. Les paliers sont interlacés en tourniquet une fois chacun mélangé
 * en interne (plutôt que concaténés bloc par bloc).
 */
