...
import { deleteTrackDiaryEntry } from '../../lib/trackDiary';
...
/** Miroir simplifié de TrackEntryMenu (web) — voir AlbumEntryMenu (mobile) pour le pattern. */
export function TrackEntryMenu({ entryId, onEdit, onDeleted }: Props) {
    ...
    <>
      ...
      <BottomSheet isOpen={open} onClose={() => setOpen(false)} title="Options" snapPoint="30%">
        <View className="px-6 py-2">
          <Pressable onPress={() => { setOpen(false); onEdit(); }} className="flex-row items-center gap-2.5 py-3 border-b border-border-divider">
            <Edit2 size={15} color="#6B6B6B" />
            <Text className="text-text-primary" style={metaStyle}>Modifier</Text>
          </Pressable>
          <Pressable onPress={() => { setOpen(false); setConfirmDelete(true); }} className="flex-row items-center gap-2.5 py-3">
