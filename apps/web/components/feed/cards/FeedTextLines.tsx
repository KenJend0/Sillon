...
export function FeedTextLines({
  ...
  const lines: { content: ReactNode; text?: string; cls: string }[] = [
    { content: context, text: contextText, cls: 'text-label text-text-tertiary leading-tight lg:text-[12.5px]' },
  ];
  if (title) {
    lines.push({ content: title, text: titleText, cls: 'mt-0.5 text-meta leading-snug font-display italic text-text-warm lg:text-[15px]' });
  }
  if (artist) {
    lines.push({ content: artist, text: artistText, cls: 'mt-0.5 text-sm text-text-tertiary leading-snug lg:text-[14px]' });
  }
