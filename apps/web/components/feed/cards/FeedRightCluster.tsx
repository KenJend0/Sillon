...
export function FeedRightCluster({ rating, coverUrl, coverHref, coverAlt = '', stacked = false }: FeedRightClusterProps) {
  const cover = coverUrl && (
    <div className={`relative w-11 h-11 rounded-cover-sm overflow-hidden bg-background-secondary flex-shrink-0 lg:h-12 lg:w-12 ${stacked ? 'shadow-cover' : ''}`}>
      {stacked && (
  ...
  const ratingBadge = rating != null && (
    <span className="inline-flex items-baseline gap-0.5 bg-paper-hi border border-accent rounded-badge px-1.5 py-0.5 text-accent font-display italic text-[13px] leading-none flex-shrink-0 lg:text-[14px]">
      {Math.round(rating)}
  ...
  return (
    <div className="flex flex-col items-center gap-1 flex-shrink-0 ml-auto lg:gap-1.5">
