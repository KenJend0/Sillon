...
export default function PublicFeedCard({ entry }: { entry: PublicFeedEntry }) {
  ...
  if (!hasWords) {
    return (
      <div
        className="relative flex items-center gap-3 px-3 py-2 cursor-pointer lg:gap-4 lg:px-4 lg:py-3"
  ...
  return (
    <div
      className="relative rounded-card border border-accent px-3 pt-3 pb-2 cursor-pointer lg:px-4 lg:pt-4 lg:pb-3"
      onClick={(e) => handleCardNavigation(e.target)}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
