...
export function middleware(req: NextRequest) {
  ...
  // If request doesn't match guarded paths, continue
  const isProtected = MATCHER.some((m) => {
    // convert "/path/:path*" into prefix check
  ...
  const hasAuth = cookieNames.some(
    (n) =>
      candidates.includes(n) || n.startsWith("sb:") || n.startsWith("sb-") || n.includes("supabase")
  );
