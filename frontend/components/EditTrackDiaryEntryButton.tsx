      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-[#1C1C1C]/20 flex items-center justify-center z-50 p-4">
          <div className="bg-background rounded-[12px] p-6 max-w-md w-full border border-border">
            <h2 className="text-[14px] font-medium text-text-primary mb-4">Mettre à jour</h2>
            <form onSubmit={handleUpdate} className="space-y-3">
              ...
              <textarea
                ...
                rows={3}
                className="w-full border border-border rounded-[10px] p-3 text-[14px] bg-background-secondary text-text-primary placeholder-text-tertiary focus:outline-none focus:border-[#8E6F5E]"
              />
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 px-3 py-2.5 bg-[#1C1C1C] hover:opacity-85 text-[#F5F3EF] rounded-[8px] text-[14px] disabled:opacity-50 transition-opacity"
                >
                  Mettre à jour
                </button>
              </div>
